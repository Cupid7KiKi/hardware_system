...
func GetDashBoard1(ctx *context.Context) (types.Panel, error) {
	...
	col1 := template2.Default().Col().
		SetContent(infobox).            // 设置内容为infobox1
		SetSize(types.SizeXS(6).SM(3)). // 设置大小为  xs-6 sm-3
		GetContent()
	col2 := template2.Default().Col().
		SetContent(infobox2).           // 设置内容为infobox2
		SetSize(types.SizeXS(6).SM(3)). // 设置大小为  xs-6 sm-3
		GetContent()
	ro2 := components.Row().SetContent(col1 + col2).GetContent()

...
func GetInventoryTable(ctx *context.Context) table.Table {
	...
	formList.AddField("产品名称", "product_id", db.Int, form.SelectSingle).FieldOptions(service.TransFieldOptions(service.GetProducts(), "product_name", "id"))
	formList.AddField("仓库名称", "warehouse_id", db.Int, form.SelectSingle).FieldOptions(service.TransFieldOptions(service.GetWarehouse(), "name", "id"))

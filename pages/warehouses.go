...
func GetWarehousesTable(ctx *context.Context) table.Table {
	...
	info := warehouses.GetInfo()

	info.AddField("ID", "id", db.Int)
	info.AddField("仓库名称", "name", db.Varchar)
	info.AddField("仓库位置", "location", db.Varchar)
	...
	formList.AddField("仓库名称", "name", db.Varchar, form.Text)
	formList.AddField("仓库位置", "location", db.Varchar, form.Text)
	formList.AddField("仓库面积", "capacity", db.Decimal, form.Text).FieldHelpMsg("单位为㎡（平方米）")
